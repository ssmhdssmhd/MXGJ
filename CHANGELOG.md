...
### ✨ 新增：特殊资源站可通过「跟随播放链接」自定义播放入口并原样返回

- **触发条件**：资源站配置了「跟随播放链接」中转前缀（如末尾以 `=` 结尾，直接拼接地址）时，返回链接**原样输出、不再做表面/伪装包装**
  - 例：资源站「跟随播放链接」填 `http://114.134.184.91:9005/player.php?url=`，命中后返回
    `msg/url` = `http://114.134.184.91:9005/player.php?url=https://lgvideo.xyz/player/cRCj3u9a`
- 与之配合，若真实地址是 HTML 播放页，`player.php` 会内嵌 `https://vv00.xyz?url=<真实地址>` iframe 播放器，可正常打开播放（见 1.11.1）
- 实现：`mxgj_protect_url()` 增加「命中站点翻转前缀则原样返回」最高优先级；普通资源站（未配前缀）仍走「表面播放链接 / 欺诈伪装」
