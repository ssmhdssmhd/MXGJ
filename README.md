...
- 每行可配置**跟随播放链接（中转前缀）** 与 **播放入口（可选）**（如 `player.php`，仅该站命中时返回 `本站/player.php?url=真实地址`）。
...
**特殊资源站单独返回形式**：后台「资源站」每行可填**播放入口（可选）**（如 `player.php`），仅该站点命中时返回
`本站/player.php?url=真实播放地址`（明文、可内联播放），其他站点仍走默认加密表面链接 `本站/play.php?u=<base64>`。
